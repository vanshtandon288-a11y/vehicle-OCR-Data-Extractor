...
import { CheckCircle2, AlertCircle, Save, FileText, Check, Edit3 } from 'lucide-react';
      ...
      <div className="glass-card-morph p-5 rounded-2xl border border-white/10 group">
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-slate-950/60 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white font-mono focus:outline-none focus:border-indigo-500/80 focus:ring-2 focus:ring-indigo-500/30 transition-all"
          />
        </div>
      </div>
    );
    ...
    <div className="glass-card rounded-3xl p-8 shadow-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {document.documentType === DocumentType.RC && (
          <>
            {renderField('Chassis Number', 'chassisNumber', data.isChassisValid, 'e.g. MBLMC3ECBGC00871')}
            {renderField('Engine Number', 'engineNumber', data.isEngineValid, 'e.g. MC38EBEGC00802')}
            {renderField('Registration Number', 'registrationNumber', data.isRegistrationValid, 'e.g. DL6SAG2552')}
          </>
        )}

        {document.documentType === DocumentType.INSURANCE && (
          <>
            {renderField('Policy Number', 'insuranceNumber', data.isInsuranceNumberValid, 'e.g. OG241234180100001234')}
        ...
        {document.documentType === DocumentType.PUC && (
          <>
            {renderField('PUC Certificate Number', 'pucNumber', data.isPucNumberValid, 'e.g. PUC12345678')}
        ...
        {document.documentType === DocumentType.PERMIT && (
          <>
            {renderField('Permit Number', 'permitNumber', data.isPermitNumberValid, 'e.g. KA2022PER1234')}
